...
export function CallHistory({ calls, currentUserId }: CallHistoryProps) {
  ...
  return (
    <div className="divide-y divide-border">
      {calls.map((call) => {
        const other = call.conversation?.members.find((m) => m.userId !== currentUserId);
        ...
        return (
          <div key={call.id} className="flex items-center gap-3 p-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{other?.user.displayName ?? 'Unknown'}</p>
              <p className="text-xs text-muted-foreground">
                {isInitiator ? 'Outgoing' : 'Incoming'} • {formatTime(call.createdAt)}
                {call.endedAt &&
                  call.startedAt &&
                  ` • ${formatCallDuration(
            ...
            <Badge
              variant={missed ? 'destructive' : call.status === 'ENDED' ? 'secondary' : 'default'}
            >
